          ...
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="职位">
                <el-input v-model.trim="ruleForm.position" maxlength="30"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="公司">
                <!-- prop="qualification"-->
                <el-input v-model.trim="ruleForm.company" maxlength="30"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="个人介绍">
                ...
                <el-input type="textarea" placeholder="请输入内容" v-model.trim="ruleForm.introduction" maxlength="80" show-word-limit></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
...
export default {
  ...
  data() {
    var checkName = async (rule, value, callback) => {
      if (!value) return callback(new Error('请输入用户昵称'))
      if (value.length > 20) {
        return callback(new Error('用户昵称长度在 20 个字符之内'))
      }
      if (this.initUserName !== value) {
        // true:重了  false:没有重
        let { data: isRepeat } = await api.checkNickName(value)
        return isRepeat ? callback(new Error('昵称已存在')) : callback()
      }
      return callback()
    }
    return {
  ...
  methods: {
    ...
    async getuserInfo() {
      ...
      this.initUserName = res.data.nickName
    },
    // 提交
    submitForm(formName) {
