...
<template>
  <div>
    <div class="collect-answers-page" v-if="!noData">
      <p class="operation" v-if="!manageStatus&&isOwn&&collectLivesList.length>0">
        ...
        <li v-for="(item,index) in collectLivesList" :key="index" class="clearfix">
          <div>
            ...
            <div v-if="item.status!=0" class="fl clearfix qa-info" :class="[manageStatus?'check-answer':'normal-answer']">
              ...
              <div class="fl live-info" :class="{'live-infoOthers':!isOwn,'live-infoManage':manageStatus&&isOwn,'live-infoNor':(!manageStatus)&&isOwn}">
                ...
                <div class="live-btn clearfix">
                  ...
                  <div class="cancel-btn fr" v-if="!manageStatus&&isOwn">
...
export default {
  ...
  methods: {
    gotoDetails(item) {
      ...
      if (item.status != 0) {
        window.open(`/livetv/${item.id}`, '_blank')
      }
    },
    // 最赞回答
    answerlistLoad() {
