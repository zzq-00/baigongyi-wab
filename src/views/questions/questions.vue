      ...
      <ul class="qa-list-box" v-if="!noData" v-loading="answerLoading" element-loading-text="正在加载" :scroll-disabled='loadStatus' v-scroll="qaload">
        <li v-for="(item,index) in qaList" :key="index">
          ...
          <p class="tagging" v-if="item.answerCount!=0">
            <span>{{item.likeCount?item.likeCount:0}}个赞</span>
            <span>·</span>
            <span>{{item.commentCount?item.commentCount:0}}条评论</span>
          </p>
      ...
      if (!this.loginaccountId) {
        ...
        if (val == 1 || val == 3) {
          api.delAttention({ objId: objId, objType: objType }).then(res => {
            this.findSame(objId, 1, '')
          })
        } else if (val == 2) {
          api.addAttention({ objId: objId, objType: objType }).then(res => {
            this.findSame(objId, 2, res.data)
          })
        }
      }
    },
    findSame(answerAccountId, val, data) {
      for (let index = 0; index < this.qaList.length; index++) {
        let element = this.qaList[index]
        if (element.answerAccountId == answerAccountId) {
          if (val == 1) {
            element.concernStatus = 2
          } else if (val == 2) {
            element.concernStatus = data
          }
        }
      }
    }
