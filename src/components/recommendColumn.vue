<template>
  <div class="special-column">
    ...
    <ul class="column-list">
      <li v-for="(item, index) in columnList" :key="index" @click="gotoDetail('/columnDetail/'+item.id)">
        <img class="cover-img" :src="$store.state.imageDomain + item.image" alt="">
        <div>
          <h5 class="ellipsis">{{item.name}}</h5>
          <div>
            <span class="ellipsis" style="flex: 1;margin-right: 5px;" @click.stop="openTeacherHome(item.lecturerInfo.accountId)">{{item.lecturerInfo && item.lecturerInfo.realName}}</span>
            <span>
...
export default {
  ...
  methods: {
    ...
    recommendColumn(param) {
      if (param === 'prev') {
        this.columnParams.pageNum =
          this.columnParams.pageNum === 1
            ? this.columnTotalPages < 10
              ? this.columnTotalPages
              : 10
            : this.columnParams.pageNum - 1
      } else if (param === 'next') {
        this.columnParams.pageNum =
          this.columnParams.pageNum === (this.columnTotalPages < 10 ? this.columnTotalPages : 10)
            ? 1
            : this.columnParams.pageNum + 1
      }
