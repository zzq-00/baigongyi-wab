<template>
  <div>
    <el-carousel height="336px" class="self-carousel" arrow="never" :interval="8000">
      <el-carousel-item v-for="(item,index) in tasList" :key="index">
        <a :href="item.link" :style="{'pointer-events': item.link ? 'default' : 'none'}" target="_blank">
          <img :src="$store.state.imageDomain + item.pictureUrl" alt="" height="100%" width="100%">
        </a>
      </el-carousel-item>
    </el-carousel>
  </div>
