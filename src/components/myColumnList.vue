...
<template>
  <div class="column">
    <!-- 专栏列表 -->
    <ul class="classList">
      <!--@click.stop="$emit('columnDetail', item)"-->
      <li class="line" v-for="item in dataList.column.records" :key="item.accountId">
        ...
        <div class="otherInfo">
          ...
          <div class="font-12 handle">
            <template v-if="item.status===2000 || item.status===2001 || item.status===1001">
              <span style="margin-right:3px" @click.stop="pushEditColumn(item.id)">编辑</span>
              <span @click.stop="onDelete(item.id)">删除</span>
            </template>
            <template v-if="item.status===1002">
              <span v-if="!item.hasBuy" @click.stop="downColumn(item.id)">下架</span>
...
export default {
  ...
  methods: {
    ...
    pushEditColumn(id){
          this.infoWasComplete(()=>{
            if(this.dialogVisible2==false){
            this.$router.push("/myLecture/editColumn/"+id)
         }
        });
    },
    //判断讲师信息是否完善
