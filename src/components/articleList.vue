          ...
          <div>
            <h5 class="ellipsis">
              <a href="javascript:;" @click="viewDetail(item)" style="color: #333;">
                <span  style="color: #f33535;" v-if="item.deleteFlag == 1">【已删除】</span>
                <span style="color: #f33535;" v-if="item.deleteFlag == 0 && item.status == 2">【已屏蔽】</span>{{item.title}}</a>
            </h5>
            <p @click="viewDetail(item)" style="cursor:pointer">{{item.description}}</p>
            ...
            <div class="edit-button" v-if="isOwn">
              <el-button round @click="reEditArticle(item)" v-if="item.status != 2">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    viewDetail(data) {
      if(data.deleteFlag == 1) return this.$message('该文章已删除，无法查看详情')
      if (data.status == 2 && data.accountId != this.$store.state.userInfo.accountId) return this.$message("该文章已被屏蔽，无法查看详情")
      if (this.$route.path === '/myHome/myArticle/draftArt') return this.$router.push('/article/reEditArticle/' + data.id)
      window.open('/article/list/' + data.id)
    },
    openURL(url) {
      window.open(url)
    }
