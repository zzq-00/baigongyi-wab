<template>
  <div class="qa-list-page">
    <ul class="qa-list-box" v-if="!noData" v-loading="answerLoading" element-loading-text="正在加载" :scroll-disabled='loadStatus' v-scroll="qaload">
      <li v-for="(item,index) in qaList" :key="index">
        ...
        <div class="qa-person-info">
          <img v-if='item.answerCount!=0' :src="item.avatar?$store.state.imageDomain + item.avatar : require('@/assets/images/err-header-icon01.png')" alt="" @click="gotoUser(item.answerAccountId)">
          ...
          <button class="already" v-if="item.answerCount!=0 && item.concernStatus==1&&loginaccountId!=item.answerAccountId" @click="follow('1',item,index)">已关注</button>
          <button class="not" v-if="item.answerCount!=0 && item.concernStatus==2&&loginaccountId!=item.answerAccountId" @click="follow('2',item,index)">关注</button>
          <button class="already" v-if="item.answerCount!=0 && item.concernStatus==3&&loginaccountId!=item.answerAccountId" @click="follow('3',item,index)">互相关注</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    getQuestionList(id = "ebbe50e030b34863af4b08562f8592bd") {
