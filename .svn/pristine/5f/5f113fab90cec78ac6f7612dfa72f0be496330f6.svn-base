      ...
      <li class="line" v-for="item in dataList.column.records" :key="item.accountId" @click.stop="$emit('columnDetail', item)">
        ...
        <div class="otherInfo">
          <div class="photoInfo clearfix" v-if="currentPath!=='/myLecture'">
            <img @click.stop="openTeacherHome(item.lecturerInfo.accountId)" :src="item.lecturerInfo.avatar?$store.state.imageDomain + item.lecturerInfo.avatar : require('@/assets/images/err-header-icon01.png')" class="photo fl">
            <div @click.stop="openTeacherHome(item.lecturerInfo.accountId)" class="name font-12 fl">{{item.lecturerInfo.realName}}</div>
          </div>
          ...
          <div v-show="currentPath==='/myLecture'" class="font-12 handle">
            <template v-if="item.status===2000 || item.status===1001">
              <span style="margin-right:3px" @click.stop="$router.push('/myLecture/editColumn/'+item.id)">编辑</span>
...
export default {
  ...
  methods: {
    ...
    // 课程管理
    goCourseMessage(id) {
      window.open('/myLecture/courseMessage/' + id)
    },
    // 删除
    onDelete(id) {
