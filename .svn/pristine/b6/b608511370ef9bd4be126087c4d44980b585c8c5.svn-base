<template>
  <div>
    <div class="collect-answers-page" v-if="!noData">
      ...
      <ul v-loading="answerLoading" element-loading-text="正在加载" :scroll-disabled='loadStatus' v-scroll="answerlistLoad">
        <li v-for="(item,index) in collectAnswerList" :key="index" class="clearfix">
          <div>
            ...
            <div class="fl qa-info" :class="[manageStatus?'check-answer':'normal-answer']">
              <p class="list-title"><span v-if="item.questionAnswerDto.deleteFlag==1" style="color: #f33535;">【已删除】</span>{{item.question.title}}</p>
              <div class="answer-info clearfix">
                <img @click.stop="openUserCenter(item.questionAnswerDto.accountId)" :src="item.questionAnswerDto.avatar?$store.state.imageDomain + item.questionAnswerDto.avatar : require('@/assets/images/err-header-icon01.png')" alt="" class="fl">
                <div class="fl">
                  <p @click.stop="openUserCenter(item.questionAnswerDto.accountId)" style="cursor: pointer;">{{item.questionAnswerDto.nickName}}</p>
              ...
              <div class="qa-content" :class="[!item.answerWhetherItExists?'prohibit':'']" @click="gotoDetails(item)" v-if="item.questionAnswerDto.nickName && item.questionAnswerDto.deleteFlag==0">{{item.questionAnswerDto.stripHtmlContent}}</div>
              <div class="qa-content" :class="[!item.answerWhetherItExists?'prohibit':'']" @click="gotoDetails(item)" v-if="item.questionAnswerDto.nickName &&item.questionAnswerDto.deleteFlag==1">该回答已删除</div>
              <p class="tagging" v-if="item.questionAnswerDto.nickName && item.questionAnswerDto.deleteFlag==0">
...
export default {
  ...
  methods: {
    gotoDetails(item, val) {
      ...
      if (item.questionAnswerDto.deleteFlag == 0) {
