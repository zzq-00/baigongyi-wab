<template>
  <div class='fillInMessage'>
    <div class='myTeacher_nav'>
      <p>讲师入驻<span class='reject' @click='openReason' v-if='statusCode==1001'>点击查看审核驳回原因</span></p>
    </div>
          ...
          <el-col :span="24">
            <el-form-item label="讲师头像">
              ...
              <div class="upload-tx">
                ...
                <img v-if='avatarImg || $store.state.imageDomain + ruleForm.avatar' :src='avatarImg ? avatarImg : $store.state.imageDomain + ruleForm.avatar' />
              </div>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="24">
            <el-form-item label="手持证件正面照">
              ...
              <div class="upload-tx upload-sfz">
                ...
                <img v-if='sfzImg || $store.state.imageDomain + ruleForm.certificateFacade' :src='sfzImg ? sfzImg : $store.state.imageDomain + ruleForm.certificateFacade' />
              </div>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="24">
            <el-form-item label="手持证件反面照">
              ...
              <div class="upload-tx upload-sfz">
                ...
                <img v-if='sffImag || $store.state.imageDomain + ruleForm.certificateBack' :src='sffImag ? sffImag : $store.state.imageDomain + ruleForm.certificateBack' />
              </div>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="24">
            <el-form-item label="资格证书" prop="book">
              ...
              <div class="upload-tx upload-sfz" style='margin-top:11px;'>
                ...
                <img v-if='zjImg || $store.state.imageDomain + ruleForm.book2' :src='zjImg ? zjImg : $store.state.imageDomain + ruleForm.book2' />
              </div>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <div id='set-mask'>
      <el-dialog title="调整头像位置" :visible.sync="dialogTableVisible" :close-on-click-modal='false'>
        ...
        <p class="submit-img">
          <input type="button" class="btn btn-blue" value="保存" @click="finishFn('blob')">
        </p>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    myLecturerInfo() {
      api.myLecturerInfo(this.$store.state.userInfo.accountId).then(res => {
        if (res.data) {
          ...
          this.ruleForm.certificateBack = this.data.certificateBack
          this.ruleForm.certificateFacade = this.data.certificateFacade
          this.ruleForm.certificateType = this.data.certificateType.toString()
          this.ruleForm.introduction = this.data.introduction
          this.ruleForm.realName = this.data.realName
          this.ruleForm.book2 = this.data.qualification && this.data.qualification.split('&')[1]
          this.ruleForm.book = this.data.qualification && this.data.qualification.split('&')[0]
        }
      })
    },
    // 获取阿里云的授权码
      ...
      axios
        ...
        .then(res => {
          if (num === 1) {
            ...
            this.ruleForm.book2 = `images/${this.getDateFn()}${renderNum}.${type}`
          }
        })
    },
    // 头像上传图片
    ...
    submitForm(formName) {
      if (this.imgurl) {
        this.ruleForm.avatar = this.imgurl
      }
      let tipText = '';
      if(this.data.applyType==2 && this.data.status == 1002 || this.data.applyType==1 && this.data.status == 1002 || this.data.applyType==2 && this.data.status == 1001 || this.data.applyType==2 && this.data.status == 1000){
          tipText = '审核期间，其他用户仍可在您的讲师主页看到原有信息，修改后的信息审核通过后，讲师主页信息将同步更新'
      }else{
          tipText = '审核期间，信息不可修改，请确认您当前填写的资料信息准确无误'
      }
    //   if(this.data.applyType==2){
    //      tipText = '审核期间，信息不可修改，请确认您当前填写的资料信息准确无误'
    //   }else if (this.data.applyType==1) {

    //   }
      this.$refs[formName].validate(valid => {
        console.log(this.ruleForm);
        if (valid) {
          if (this.ruleForm.avatar && this.ruleForm.certificateFacade && this.ruleForm.certificateBack && this.ruleForm.book2) {
            this.$confirm(tipText, '提示', {
              ...
              this.ruleForm.qualification = this.ruleForm.book + '&' + this.ruleForm.book2
              delete this.ruleForm.book
              delete this.ruleForm.book2
